...
Puis, `*log in with OpenShift as 'userTTKN1290'*`
...
[WARNING]
====
Si vous recevez une erreur mentionnant **"Un tableau de bord dans ce dossier avec le même nom existe déjà"**, `*please add 'userTTKN1297' in the name*`.
...
* [Red] one filter selection block dans lequel nous sélectionnons un nomspace [Pink] here chaos-ingénierie{USER_ID} et un service [Cyan]  de l'application Travel
...
*<MISSING TTKN13244>xx et 5xx = Total des requêtes dans Erreur HTTP 4xx et 5xx
...
* [Pink]  Taux de réussite = > pour identifier l'erreur HTTP WHEN survienne dans l'appel de service
