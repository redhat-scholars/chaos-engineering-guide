...
Puis, connectez-vous avec OpenShift comme utilisateur {USER_ID}/{OPENSHIFT_PASSWORD}'
image::kiali-login.png[Kiali- Log In,300]

Sélectionnez la vue **'Graph'**, depuis la barre de menu latérale et saisissez la configuration suivante:
...
| Graphique de type
|**Graphique d'application vérifié**

| Afficher
|**'Response Time'** vérifié
...
Dans le {KIALI_URL}[Kiali Console^, role='params-link'], de la vue **'Services'**, cliquez sur le service 'contrôle' Actions'
image::kiali-control-request-routing.png[Kiali - Control Request Routing , 800]

Cliquez sur le bouton « Ajouter la règle »* pour rediriger tout le trafic d'entrée vers le service « contrôle »:
...
|Gateway Hosts
|**contrôle-chaos-ingénierie{USER_ID}.{APPS_HOSTNAME_SUFFIX}**
<MISSING <MISSING TTKN12>>
|===

image::kiali-control-create-ingress.png[Kiali - Control Create Ingress , 500]

Enfin, cliquez sur le bouton 'Créer'. Vous pouvez maintenant accéder au tableau de bord du contrôle des voyages en utilisant l'URL suivante, http://control-chaos-engineering{USER_ID}.{APPS_HOSTNAME_SUFFIX}[^, role='params-link'].
