...
== Create the Global Success Rate Metric

After creating the metric of all requests, you are going to define a **metric of all successfull requests** (~ error code 5XX) :
...
.PromQL
----
sum(increase(istio_requests_total{reporter="source", namespace="chaos-engineering{USER_ID}", response_code!~"5.*"}[1m]))/ sum(rate(istio_requests_total{reporter="source", namespace="chaos-engineering{USER_ID}"}[1m]))
----
